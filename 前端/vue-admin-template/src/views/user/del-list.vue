<template>
  <div class="app-container">
    ...
    <div class="controls">
      ...
      <div class="left-botton">
        <el-row>
          ...
          <el-tooltip class="item" effect="light" content="批量恢复" placement="top-start">
            <el-button type="danger" icon="el-icon-refresh-left" circle @click="handleRefreshAll" />
          </el-tooltip>

          <el-tooltip class="item" effect="light" content="导出数据" placement="top-start">
              <el-button type="success" icon="el-icon-download" circle @click="handleExportXls('已被删除的用户')"/>
          </el-tooltip>
    ...
    >
      ...
      <el-table-column align="center" show-overflow-tooltip prop="delBy" label="删除人" width="120" />
      <el-table-column align="center" show-overflow-tooltip prop="delTime" label="删除时间" width="120">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" fixed="right" label="操作" width="150">
        <template slot-scope="scope">
          ...
          <el-button type="text" size="small" @click="handleRefresh(scope.row)">恢复</el-button>
...
export default {
  ...
  methods: {
    ...
    // 恢复用户
    handleRefresh(val) {
      MessageBox.confirm('确定恢复该用户？', '恢复用户', {
    ...
    handleRefreshAll() {
      if (this.ids === '') {
        ...
        MessageBox.confirm('您确定恢复选中用户吗？', '批量恢复确认', {
          confirmButtonText: '确认恢复',
          cancelButtonText: '取消恢复',
          type: 'warning'
        }).then((action) => {
          // 确认按钮的回调
          if (action === 'confirm') {
            refreshAllUser(this.ids).then((res) => {
              if (res.success === true) {
                // 刷新用户列表
                this.handleGetUserList(this.pageCurrent, this.pageSize)
                Message({
    ...
    handleExportXls(fileName){
      ...
      exportDelUser(fileName).then((res) => {
          //
    ...
    handleGetUserList(pageCurrent, pageSize, keyword, value) {
      getDelUserList(pageCurrent, pageSize, keyword, value).then((res) => {
